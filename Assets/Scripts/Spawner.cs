...
public class Spawner : MonoBehaviour {
    ...
    public void SpawnLeft()
    {
        ...
        {
            Vector3 spawnPoint = new Vector3(SpawnPoint_1.position.x, SpawnPoint_1.position.y + (gap * i), SpawnPoint_1.position.z);
    ...
    public void SpawnRight()
    {
        ...
        for (int i = 0; i < amount; i++)
        {
            Vector3 spawnPoint = new Vector3(SpawnPoint_2.position.x, SpawnPoint_2.position.y + (gap * i), SpawnPoint_2.position.z);
            GameObject obj = Instantiate(Objects[spawnIndex], spawnPoint, SpawnPoint_1.rotation);
            obj.GetComponent<Rigidbody2D>().gravityScale = Random.Range(min_speed, max_speed);
        }
    }
